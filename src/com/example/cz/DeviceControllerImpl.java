...
import com.example.cz.AppExCode;
import com.example.controll.DeviceController;
import com.example.cz.Const.DataEncryptWKIndexConst;
import com.example.cz.Const.KeyIndexConst;
import com.example.cz.Const.MKIndexConst;
import com.example.cz.Const.MacWKIndexConst;
...
public class DeviceControllerImpl implements DeviceController {
	...
	public static DeviceControllerImpl impl = new DeviceControllerImpl();
	...
	public void loadMainKey(KekUsingType kekUsingType, int mkIndex, byte[] keyData, byte[] checkValue, int transportKeyIndex) {
		...
		byte[] rslt = pinInput.loadMainKey(kekUsingType, mkIndex, keyData, transportKeyIndex);
		...
		System.out.println("checkvalue:"+"failed to check kcv!:[" + Dump.getHexDump(expectedKcv) + "," + Dump.getHexDump(checkValue) + "]");
	}
	@Override
	...
	public SwipResult swipCard(String msg, long timeout, TimeUnit timeUnit) {
		...
		try {
			...
			case COMMON_SWIPER:
				...
				SwipResult swipRslt = getSwipResult(swiper, DataEncryptWKIndexConst.DEFAULT_TRACK_WK_INDEX, TrackEncryptAlgorithm.BY_UNIONPAY_MODEL);
	...
	public ME11SwipResult swipCard_me11(byte[] time, long timeout, TimeUnit timeUnit) {
		ME11External me11Model = (ME11External) deviceManager.getDevice().getExModule(ME11External.MODULE_NAME);
		ME11SwipResult swipeResult = me11Model.openCardReader(new ModuleType[] { ModuleType.COMMON_SWIPER, ModuleType.COMMON_ICCARD }, timeout, timeUnit,
				new SwiperReadModel[] { SwiperReadModel.READ_SECOND_TRACK, SwiperReadModel.READ_THIRD_TRACK }, (byte)0xff, 
				TrackEncryptAlgorithm.BY_UNIONPAY_MODEL, new WorkingKey(DataEncryptWKIndexConst.DEFAULT_TRACK_WK_INDEX), time, null, null);
		return swipeResult;
	}
	@Override
	public ME11SwipResult swipCardForPlain_me11(byte[] time, long timeout, TimeUnit timeUnit) {
		...
		ME11SwipResult swipeResult = me11Model.openCardReader(new ModuleType[] { ModuleType.COMMON_SWIPER, ModuleType.COMMON_ICCARD }, timeout, timeUnit,
				new SwiperReadModel[] { SwiperReadModel.READ_SECOND_TRACK, SwiperReadModel.READ_THIRD_TRACK }, (byte)0xff, 
				TrackEncryptAlgorithm.BY_PLAIN_MODEL, new WorkingKey(DataEncryptWKIndexConst.DEFAULT_TRACK_WK_INDEX), time, null, null);
		return swipeResult;
	}
	...
	public void startEmv(BigDecimal amt, TransferListener transferListener) {
		...
		try {
				...
				config.setAid(ISOUtils.intToBytes(0x9f06, true));
				boolean issuccessaid = module.addAID(config);
...
//						listener
//				config.setWorkingKey(new WorkingKey(KeyIndexConst.KSN_INITKEY_INDEX));
...
//			cardReader.closeCardReader();
	...
	public PinInputEvent startPininput(String acctHash, int inputMaxLen, String msg) {
		...
		PinInput pinInput = (PinInput) deviceManager.getDevice().getStandardModule(ModuleType.COMMON_PININPUT);
		PinInputEvent event = pinInput.startStandardPinInput(new WorkingKey(PinWKIndexConst.DEFAULT_PIN_WK_INDEX), PinManageType.MKSK, AccountInputType.USE_ACCT_HASH, acctHash, inputMaxLen, new byte[] { 'F', 'F', 'F', 'F', 'F', 'F', 'F', 'F', 'F', 'F' }, true, msg, 30, TimeUnit.SECONDS);
		return event;
	}
	@Override
	...
	public byte[] encrypt(WorkingKey wk, byte[] input) {
		PinInput pinInput = (PinInput) deviceManager.getDevice().getStandardModule(ModuleType.COMMON_PININPUT);
		return pinInput.encrypt(wk, EncryptType.ECB, input, new byte[] { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 });
	}

	@Override
	public byte[] caculateMac(byte[] input) {
		PinInput pinInput = (PinInput) deviceManager.getDevice().getStandardModule(ModuleType.COMMON_PININPUT);
		return pinInput.calcMac(MacAlgorithm.MAC_ECB, new WorkingKey(MacWKIndexConst.DEFAULT_MAC_WK_INDEX), input);
	}
	...
	public String inputPlainPwd(String title, String content, int minLength, int maxLength, long timeout) throws InterruptedException {
		...
		keyboard.readPwd(DispType.NORMAL, title, content, minLength, maxLength, (int) timeout, TimeUnit.SECONDS, listener);
		try {
			listener.startWait();
		} catch (InterruptedException e) {
